		...
		>
			...
			<template v-for="(item, index) in option.column">
				...
				>
					<template #="{ row }">
						<span
							v-html="getFormatter(item, row)"
							v-if="item.formatter && reload"
						></span>
...
export default create({
	...
	computed: {
		...
		cellHeight() {
			return parseInt(
				(this.height - this.headerHeight) / this.option.count
			);
		},
	},
	props: {
	...
	methods: {
		...
		setTime() {
			...
			const divData = table.$refs.bodyWrapper.querySelector(
				".el-scrollbar__wrap"
			);
			...
			if (this.scroll) {
				this.scrollCheck = setInterval(() => {
					...
					if (top >= this.cellHeight && this.scrollTime) {
						divData.scrollTop =
							divData.scrollTop - (top - this.cellHeight);
						clearInterval(this.scrollCheck);
						setTimeout(() => {
							this.setTime();
						}, this.scrollTime);
		...
		cellStyle({ row, column, rowIndex, columnIndex }) {
			return {
				...
				textAlign:
					column.type == "index"
						? "center"
						: this.option.bodyTextAlign,
				backgroundColor:
					rowIndex % 2 == 0
						? this.option.othColor
						: this.option.nthColor,
			};
		},
		rowStyle({ rowIndex }) {
			return {
