	...
	<div class="page">
		...
		<div class="jp-table">
			...
			>
				<template #buttons>
					<el-button
						v-if="hasPermission('extension:condition:add')"
						type="primary"
						icon="plus"
						@click="add()"
						>新建</el-button
					>
					<el-button
						v-if="hasPermission('extension:condition:edit')"
						type="warning"
						icon="edit-filled"
						@click="edit()"
						v-show="
							$refs.conditionTable &&
							$refs.conditionTable.getCheckboxRecords().length ==
								1
						"
						>修改</el-button
					>
					<el-button
						v-if="hasPermission('extension:condition:del')"
						type="danger"
						icon="del-filled"
						@click="del()"
						v-show="
							$refs.conditionTable &&
							$refs.conditionTable.getCheckboxRecords().length > 0
						"
						>删除</el-button
					>
				</template>
			...
			<div class="jp-table-body">
				...
				>
					...
					<vxe-column title="表达式" field="expression" sortable>
					</vxe-column>
					...
					>
						<template #default="scope">
							<el-button
								v-if="hasPermission('extension:condition:view')"
								type="primary"
								text
								icon="view-filled"
								@click="view(scope.row.id)"
...
export default {
	...
	mounted() {
		this.$nextTick(() => {
			...
			const $toolbar = this.$refs.conditionToolbar;
	...
	methods: {
		...
		edit(id) {
			id =
				id ||
				this.$refs.conditionTable.getCheckboxRecords().map((item) => {
					return item.id;
				})[0];
