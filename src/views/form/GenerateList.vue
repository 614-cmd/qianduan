<template>
	<div class="page">
      ...
      >
				...
				<template #buttons>
					<el-button
						v-if="
							...
							hasPermission(`form:${tableName}:add`)
						"
						type="primary"
						icon="plus"
						@click="add()"
					...
					<el-button
						v-if="
							$route.query.previewMode ||
							hasPermission(`form:${tableName}:edit`)
						"
						type="warning"
						icon="edit-filled"
						@click="edit()"
					...
					<el-button
						v-if="
							$route.query.previewMode ||
							hasPermission(`form:${tableName}:del`)
						"
						type="danger"
						icon="del-filled"
						@click="del()"
						v-show="
							$refs.table &&
							$refs.table.getCheckboxRecords().length > 0
						"
						>删除
					</el-button>
					<el-button
					...
					>
						<template #default="scope">
							...
							<el-button
								v-for="(
									item, index
								) in options.config.actions.filter((action) => {
									return (
		...
		<!-- 导入导出-->
		<el-dialog title="导入Excel" v-model="isImportCollapse" draggable>
			<el-form :inline="true" class="query-form m-b-10" ref="importForm">
				<el-form-item>
					<el-button type="default" @click="downloadTpl()"
						>下载模板</el-button
					>
				</el-form-item>
...
<script>
import UserSelect from "@/components/userSelect"; //jeeplus
import AreaSelect from "@/components/areaSelect"; //jeeplus
...
export default {
	...
	activated() {
		makeFormService.queryById(this.$route.query.id).then((data) => {
			if (data.source) {
				...
				//  this.options =  {"list":[],"config":{"labelWidth":100,"labelPosition":"right","size":"default","customClass":"","ui":"element","layout":"horizontal","width":"100%","hideLabel":false,"hideErrorMessage":false,"eventScript":[{"key":"mounted","name":"mounted","func":""},{"key":"refresh","name":"refresh","func":""}],"dataSource":[]}}
			}
			...
			this.$nextTick(() => {
				...
				const $toolbar = this.$refs.tableToolbar;
	...
	methods: {
		...
		go(item, row) {
			row =
				row ||
				this.$refs.table.getCheckboxRecords().map((item) => {
					return item;
				})[0];
			let keyValue = item.paramValue;
			if (/^[$]{.*}$/.test(item.paramValue)) {
				let keyName = item.paramValue.match(
					/[$]{[a-zA-Z0-9]*[.]+(\S*)}/
				)[1];
		...
		del(id) {
			...
			this.$confirm(`确定删除所选项吗?`, "提示", {
				...
				this.loading = true;
				generateFormService
					.delete({ formId: this.$route.query.id, ids: ids })
					.then((data) => {
						this.loading = false;
