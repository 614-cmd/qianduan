<template>
  <div class="page">
    ...
    >
      <!-- 搜索框-->
      <el-form-item prop="office.id" label="归属部门：">
				<office-select   v-model="searchForm.office.id" type="2"/>
		   </el-form-item>
      <el-form-item prop="user.id" label="员工：">
      ...
      <el-form-item>
		...
		<div class="jp-table">
		  <vxe-toolbar ref="testFormLeaveToolbar" :refresh="{query: refreshList}" import export print custom>
		    <template #buttons>
				<el-button v-if="hasPermission('test:one:testFormLeave:add')" type="primary"  icon="plus" @click="add()">新建</el-button>
				<el-button v-if="hasPermission('test:one:testFormLeave:edit')" type="warning" icon="edit-filled" @click="edit()" v-show="$refs.testFormLeaveTable && $refs.testFormLeaveTable.getCheckboxRecords().length === 1" plain>修改</el-button>
				<el-button v-if="hasPermission('test:one:testFormLeave:del')" type="danger"   icon="del-filled" @click="del()" v-show="$refs.testFormLeaveTable && $refs.testFormLeaveTable.getCheckboxRecords().length > 0" plain>删除</el-button>
		    </template>
		    <template #tools>
		<vxe-button
		  type="text"
  ...
  :export-config="{
    ...
    <vxe-column
		...
		<vxe-column
		  ...
		  title="操作">
		  <template   #default="{ row }">
		    <el-button v-if="hasPermission('test:one:testFormLeave:view')" type="primary" text icon="view-filled" @click="view(row.id)">查看</el-button>
		    <el-button v-if="hasPermission('test:one:testFormLeave:edit')" type="primary" text icon="edit-filled" @click="edit(row.id)">修改</el-button>
		    <el-button v-if="hasPermission('test:one:testFormLeave:del')"  type="danger"  text icon="del-filled" @click="del(row.id)">删除</el-button>
		  </template>
		</vxe-column>
    </vxe-table>
    <vxe-pager
...
<script>
  ...
  export default {
    ...
    importMethod({ file }) {
		...
		this.loading = true
		testFormLeaveService.importExcel(formBody).then((result) => {
