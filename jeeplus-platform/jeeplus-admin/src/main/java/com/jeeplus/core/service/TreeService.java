...
public abstract class TreeService<D extends TreeMapper <T>, T extends TreeEntity <T>> extends ServiceImpl <D, T> {
    ...
    public boolean removeWithChildrenById(String id) {
        LambdaQueryWrapper <T> queryWrapper = new LambdaQueryWrapper <> ( entityClass ).like ( StrUtil.isNotBlank ( id ), T::getParentIds, "," + id + "," ).or ( ).eq ( StrUtil.isNotBlank ( id ), T::getId, id );
    ...
    public List <T> treeData(String extId) {
        List <T> allList = super.list ( new LambdaQueryWrapper <> ( entityClass ).orderByAsc ( T::getSort ) );
        try {
