...
public class QueryWrapperGenerator {
    ...
    public static <T> QueryWrapper <T> buildQueryCondition(T searchObj, Class <T> entityClass) throws Exception {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if ( field.isAnnotationPresent ( MultiQuery.class ) ) {
                MultiQuery annotation = field.getAnnotation ( MultiQuery.class );
                Query[] queryList = annotation.value ( );
                for (Query query : queryList) {
                    queryWrapper =  buildQueryWrapper ( field, queryWrapper, searchObj, query );
                }
    ...
    private static  <T> QueryWrapper buildQueryWrapper(Field field, QueryWrapper <T> queryWrapper, T searchObj, Query annotation) throws Exception{
