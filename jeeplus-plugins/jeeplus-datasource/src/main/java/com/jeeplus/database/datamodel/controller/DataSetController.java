...
public class DataSetController {
    ...
    public ResponseEntity delete(@RequestParam(value = "ids") String ids) {
        ...
        for (String id : idArray) {
            List <Map <String, Object>> list = jdbcTemplate.queryForList ( "select name from plugin_echarts where data_set_id = '" + id + "'" );
            if ( list.size ( ) > 0 ) {
                String name = "";
                for (Map <String, Object> map : list) {
                    name = name + " [" + map.get ( "name" ).toString ( ) + "]";
                }
                msg.append ( "数据模型 [" + dataSetService.getById ( id ).getName ( ) + "] 已被图表" + name + " 使用，无法删除。请先删除图表，再删除该数据模型。" );
            } else {
                ...
                msg.append ( "删除数据模型 [" + dataSetService.getById ( id ).getName ( ) + "] 成功。" );
    ...
    public ResponseEntity getData(@PathVariable("id") String id, @PathVariable("type") String type, HttpServletRequest request) {
        ...
        try {
            JdbcTemplate jdbcTemplate = DBPool.getInstance ( ).getJdbcTemplateByDsId ( dataSet.getDataSourceId ( ) );
            if ( jdbcTemplate == null ) {
                return ResponseEntity.badRequest ( ).body ( "数据库链接不存在!" );
            }
