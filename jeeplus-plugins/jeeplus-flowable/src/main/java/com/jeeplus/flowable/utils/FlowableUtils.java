...
import com.jeeplus.flowable.common.cmd.UpdateHiTaskInstCmd;
import com.jeeplus.flowable.common.constant.FlowableConstant;
import com.jeeplus.flowable.model.Flow;
import com.jeeplus.flowable.vo.MultiInstanceVO;
...
public class FlowableUtils {
    ...
    public static String getFormUrl(String formKey, Flow flow) throws Exception {
        ...
        formUrl.append ( "&act.taskName=" ).append ( flow.getTaskName ( ) != null ? URLEncoder.encode ( flow.getTaskName ( ), StandardCharsets.UTF_8 ) : "" );
    ...
    public static boolean isReachable(Process process, FlowNode sourceElement, FlowNode targetElement,
        ...
        // trigger)
        if ( sourceElement instanceof StartEvent && sourceElement.getParentContainer() instanceof SubProcess) {
            return false;
        }
    ...
    public static List <String> getParentProcessIds(FlowNode flowNode) {
        ...
        FlowElementsContainer flowElementsContainer = flowNode.getParentContainer ( );
        while (flowElementsContainer != null) {
            if ( flowElementsContainer instanceof SubProcess flowElement ) {
                result.add ( flowElement.getId ( ) );
                flowElementsContainer = flowElement.getParentContainer ( );
            } else if ( flowElementsContainer instanceof Process flowElement ) {
                result.add ( flowElement.getId ( ) );
                flowElementsContainer = null;
            }
        }
    ...
    public static MultiInstanceVO isMultiInstance(String processDefinitionId, String taskDefinitionKey) {
        BpmnModel bpmnModel = SpringUtil.getBean( ProcessEngine.class).getRepositoryService().getBpmnModel(processDefinitionId);
    ...
    public static TaskEntity createNewTask(Task currentTask) {
        ...
        if ( ObjectUtil.isNotEmpty(currentTask)) {
            task = (TaskEntity)  SpringUtil.getBean( ProcessEngine.class).getTaskService ().newTask();
            ...
            SpringUtil.getBean( ProcessEngine.class).getTaskService ().saveTask(task);
        }
        if (ObjectUtil.isNotNull(task)) {
            ...
            SpringUtil.getBean( ProcessEngine.class).getManagementService().executeCommand(updateHiTaskInstCmd);
        }
        return task;
    }
