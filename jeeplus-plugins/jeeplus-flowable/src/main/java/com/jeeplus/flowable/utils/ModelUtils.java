...
public class ModelUtils {
    ...
    public static byte[] bpmnJsonToXmlBytes(byte[] jsonBytes) throws IOException {
        ...
        BpmnModel bpmnModel = new FlowBpmnJsonConverter ().convertToBpmnModel(jsonNode);
    ...
    private static void checkBpmnNode(Collection<FlowElement> flowElements, boolean subtask) throws ServerException {
        ...
        if ((targetFlowElement instanceof UserTask userTask) && !subtask) {
            if (StrUtil.isBlank(userTask.getFormKey())) {
    ...
    public static ModelKeyRepresentation validateModelKey(Model model, String key) {
        ModelKeyRepresentation modelKeyResponse = new ModelKeyRepresentation();
        modelKeyResponse.setKey(key);

        List<Model> models =   PROCESS_ENGINE.getRepositoryService ().createModelQuery ().modelKey ( key ).list ();
