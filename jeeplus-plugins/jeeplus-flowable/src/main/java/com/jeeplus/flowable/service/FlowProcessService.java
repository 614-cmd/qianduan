...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
    ...
    public void stopProcessInstanceById(String procInsId, ProcessStatus processStatus, String comment) {
        ...
        if ( processInstance != null ) {
            List<Task> currentTasks = taskService.createTaskQuery ( ).processInstanceId ( procInsId ).list ( );
            //1、添加审批记录
            ...
            if(currentTasks.size () > 0){
                taskService.addComment ( currentTasks.get ( 0 ).getId ( ), procInsId, taskComment.getCommentType ( ), taskComment.getFullMessage ( ) );
                if ( StrUtil.isBlank ( currentTasks.get ( 0 ).getAssignee ( ) ) ) { // 未签收任务
                    taskService.claim ( currentTasks.get ( 0 ).getId ( ), UserUtils.getCurrentUserDTO ( ).getId ( ) );
                }
            }
