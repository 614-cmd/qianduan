...
public class FlowableTaskController {
    ...
    public ResponseEntity myAppliedListData(Page <ProcessVO> page, Flow flow) throws Exception {
    ...
    public ResponseEntity getTaskDef(Flow flow) {
        ...
        String formType = "1";  // 1 代表动态表单， 2 代表外置表单
        ...
        if ( StrUtil.isNotBlank (flow.getProcInsId ( )) ) {
            if ( flowTaskService.getProcIns ( flow.getProcInsId ( ) ) != null ) {
                ProcessInstance processInstance = flowTaskService.getProcIns ( flow.getProcInsId ( ) );
                flow.setProcIns ( processInstance );
                flow.setTitle ( processInstance.getProcessVariables ().get ( "title" ).toString () );
            } else {
                HistoricProcessInstance processInstance = flowTaskService.getFinishedProcIns ( flow.getProcInsId ( ) );
                flow.setFinishedProcIns ( processInstance );
                flow.setTitle ( processInstance.getProcessVariables ().get ( "title" ).toString () );
            }
        }
        if(StrUtil.isNotBlank ( flow.getProcDefId () )){
            flow.setProcDef ( repositoryService.getProcessDefinition ( flow.getProcDefId () ));
        }
        //获取任务节点实例
        if( StrUtil.isNotBlank (flow.getTaskId ( ))){
     ...
     */
    @ApiLog("读取流程历史数据")
    @Operation(summary = "读取流程历史数据")
    @GetMapping("getFlowChart")
    public Map getFlowChart(String processInstanceId) {
    ...
    public ResponseEntity addSignTask(@RequestBody AddMultiTaskVO addMultiTask) {
        flowTaskService.addMultiInstanceExecution ( addMultiTask  );
     ...
     */
    @ApiLog("获取会签节点的处理人")
    @Operation(summary = "获取会签节点的处理人")
    @GetMapping("/getMultiInstanceList")
    public ResponseEntity getMultiInstanceList(String taskId) {
        List <TaskVO> tasks = flowTaskService.getMultiInstanceList ( taskId ).stream( ).map ( task -> new TaskVO ( task ) ).toList ();
    ...
    @PostMapping("delSignTask")
    public ResponseEntity delSignTask(@RequestBody List<DeleteMultiTaskVO> deleteMultiTasks) {
        flowTaskService.deleteMultiInstanceExecution (deleteMultiTasks);
        return ResponseEntity.ok ( "减签成功!" );
    }
    ...
    public ResponseEntity <String> urge(String taskId, String content) {
        List <UserDTO> assignList = Lists.newArrayList ( );
        taskService.getIdentityLinksForTask ( taskId ).forEach ( identityLink -> {
            assignList.add ( UserUtils.get ( identityLink.getUserId ( ) ) );
        } );
        mailService.sendEmail ( assignList, "你收到一条流程催办任务!", content + "<p><a href=\"/#/flowable/task/TodoList\" target=\"_blank\">点击处理</a><br></p>" );
