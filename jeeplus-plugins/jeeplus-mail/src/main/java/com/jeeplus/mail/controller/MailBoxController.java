...
public class MailBoxController {
    ...
    public ResponseEntity queryStatus() {
        ...
        //查询未读的条数
        long noReadCount = mailBoxService.lambdaQuery ( ).eq ( MailBox::getReadStatus, "0" ).eq ( MailBox::getReceiverId, UserUtils.getCurrentUserDTO ( ).getId ( ) ).count ( );
        responseUtil.add ( "noReadCount", noReadCount );

        //查询总条数
        long mailBoxCount = mailBoxService.lambdaQuery ( ).eq ( MailBox::getReceiverId, UserUtils.getCurrentUserDTO ( ).getId ( ) ).count ( );
        responseUtil.add ( "mailBoxCount", mailBoxCount );

        //查询已发送条数
        long mailComposeCount = mailComposeService.lambdaQuery ( ).eq ( MailCompose::getStatus, "1" ).eq ( MailCompose::getSenderId, UserUtils.getCurrentUserDTO ( ).getId ( ) ).count ( );
        responseUtil.add ( "mailComposeCount", mailComposeCount );


        //查询草稿箱条数
        long mailDraftCount = mailComposeService.lambdaQuery ( ).eq ( MailCompose::getStatus, "0" ).eq ( MailCompose::getSenderId, UserUtils.getCurrentUserDTO ( ).getId ( ) ).count ( );
        responseUtil.add ( "mailDraftCount", mailDraftCount );

        //查询垃圾箱数
        long mailTrashCount = mailTrashService.lambdaQuery ( ).eq ( MailTrash::getCreateById, UserUtils.getCurrentUserDTO ( ).getId ( ) ).count ( );
